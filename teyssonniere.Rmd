...
rna = read.delim("/Users/carloalbertozani/Documents/Internship/teyssonniere/RNAseq_969_log2tpm_matrix.tab", header = TRUE, sep = ",", stringsAsFactors = FALSE)
protein = read.delim("/Users/carloalbertozani/Documents/Internship/teyssonniere/DIA-NN_1.8/SCmedia_MBR_CommonPeptide_Approach/211025_ProteomicsData_SCmediaMBR_DIA-NN-1.8_genes_imputed_KNN_ORF.tsv")

rna <- rna %>%
  filter(str_starts(ORF, "Y"))

key_file <- read.csv("/Users/carloalbertozani/Documents/Internship/teyssonniere/FinalStrainInfo1040.csv", sep = ";")

gene_list <- read_excel('/Users/carloalbertozani/ownCloud/Carlo/Internship Project/Keyfiles/514_MitoGenes_then_55Glycogluco_Genes.xlsm')
...
# # Count occurrences of each Eco_Origin category
# eco_origin_counts <- key_file_alcohol %>%
#   dplyr::count(Eco_Origin, sort = TRUE)
# 
# # Create the count plot with different colors for high and low alcohol strains
# ggplot(key_file_alcohol, aes(x = fct_infreq(Eco_Origin), fill = Expression_Group)) +
#   geom_bar(color = "black", position = "dodge") +
#   theme_classic() +
#   labs(title = " ",
#        x = "Eco_Origin",
#        y = "Count",
#        fill = "Strain Specific Relative Expression") +
...
library(gridExtra)  # or use patchwork package
...
#   stat_compare_means(comparisons = list(c("Sake", "Bioethanol")), 
#                      label = "p.format", 
#                      method = "t.test",  # Change to "wilcox.test" for non-parametric
#                      size = 5, 
#                      y.position = max(expression_long$Expression, na.rm = TRUE) + 0.5)
```


# DEA. Interesting: dairy, bioethanol, sake
```{r}
...
# Example usage for three comparisons
...
# Next steps: map back from the ENTREZID to the ORF and then collect a list of the genes found significantly expressed in those pathways
```{r}
...
# Results: 65 genes with no log filter, 10 with log filter of 0.5

gene_list_file <- read_excel('/Users/carloalbertozani/ownCloud/Carlo/Internship Project/Keyfiles/Additions to the gene list.xlsx')

gene_list_file <- gene_list_file %>%
  filter(`Gene name` != "N/A") %>%
  dplyr::select(-`Reason for addition`)

```


# Correlation between gene expression and proteins
```{r pro-gene corr, echo=FALSE}
  # Filter rows where ORF matches Protein.Group
rna_protein_matched <- rna %>%
  filter(ORF %in% protein$Protein.Group)

protein_matched <- protein %>%
  filter(Protein.Group %in% rna_protein_matched$ORF)

rownames(rna_protein_matched) <- rna_protein_matched$ORF
rownames(protein_matched) <- protein_matched$Protein.Group

# Match columns between RNA and protein data
common_columns <- intersect(colnames(rna_protein_matched), colnames(protein_matched))

# Subset the RNA and protein data to only include matching columns
rna_protein_matched <- rna_protein_matched[, common_columns]
protein_matched <- protein_matched[, common_columns]

gen_prot_cor <- cor(t(rna_protein_matched), t(protein_matched), method = "spearman")

sample_cor <- cor(rna_protein_matched, protein_matched, method = "spearman")

# Create the data frame
results_genes <- data.frame(diag(gen_prot_cor))

results_samples <- data.frame(diag(sample_cor))

# Median for the correlation distribution
median_correlation <- median(results_genes$diag.gen_prot_cor., na.rm = TRUE)

median_samples <- median(results_samples$diag.sample_cor., na.rm = TRUE)

# Create the density plot
ggplot(results_genes, aes(x = diag.gen_prot_cor.)) +
  geom_density(fill = "blue", alpha = 0.5) +  # Density plot with a blue fill and some transparency
  geom_vline(xintercept = median_correlation, color = "red", linetype = "dashed", size = 1) +  # Add median line
  labs(
    title = "Gene-Protein Correlation",
    x = "Correlation",
    y = "Density",
    caption = paste("Median Correlation:", round(median_correlation, 3))
  ) +
  theme_minimal()

# Create the density plot
ggplot(results_samples, aes(x = diag.sample_cor.)) +
  geom_density(fill = "blue", alpha = 0.5) +  # Density plot with a blue fill and some transparency
  geom_vline(xintercept = median_samples, color = "red", linetype = "dashed", size = 1) +  # Add median line
  labs(
    title = "Sample Correlation",
    x = "Correlation",
    y = "Density",
    caption = paste("Median Correlation:", round(median_samples, 3))
  ) +
  theme_minimal()
```
